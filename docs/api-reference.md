...
Applies gain to the input signal. The `gain` parameter accepts either a numeric value or a **Signal** (e.g. from `useEnvelope`) for amplitude modulation.

| Param    | Type               | Default | Description                                              |
| -------- | ------------------ | ------- | -------------------------------------------------------- |
| `gain`   | `number \| Signal` | —       | Linear gain (0–1+), or a Signal for amplitude modulation |
| `gainDb` | `number?`          | —       | Gain in decibels (alternative to numeric `gain`)         |
| `bypass` | `boolean?`         | `false` | Bypass processing                                        |

When `gain` is a Signal, it is connected as a modulation input to the native GainNode. The audio is multiplied sample-by-sample by the modulation signal. Example with an envelope:

```tsx
const env = useEnvelope(midi, { attack: 10, decay: 200, sustain: 0.7, release: 300 });
const shaped = useGain(oscillator, { gain: env });
```
...
#### `useConvolver(input: Signal, params?: ConvolverParams): Signal`
Convolution reverb using an impulse response. The native `ConvolverNode` uses JUCE's uniformly-partitioned convolution engine for efficient frequency-domain processing. The IR must be loaded on the C++ side via `loadIR()` or `loadIRFromFile()`.

| Param    | Type       | Default | Description          |
| -------- | ---------- | ------- | -------------------- |
| `mix`    | `number?`  | `0.5`   | Dry/wet mix (0–1)    |
| `gain`   | `number?`  | `1`     | Output gain (linear) |
| `bypass` | `boolean?` | `false` | Bypass               |
...
#### `useChannelSplit(input: Signal): { left: Signal; right: Signal }`
Splits a stereo signal into separate left/right mono signals.

- `left` — Left channel (outlet 0)
- `right` — Right channel (outlet 1)

#### `useChannelMerge(left: Signal, right: Signal): Signal`
Combines two mono signals into a stereo signal. The `left` signal is routed to channel 0 (inlet 0) and `right` to channel 1 (inlet 1).
...
#### `usePolyphony(options?: PolyphonyOptions): PolyphonyState`
Manages polyphonic voice allocation from MIDI input. Tracks `noteOn`/`noteOff` events and allocates voices up to `maxVoices`. When all voices are in use, applies a voice-stealing strategy.

**PolyphonyOptions:**
| Field       | Type                     | Default    | Description                       |
| ----------- | ------------------------ | ---------- | --------------------------------- |
| `maxVoices` | `number?`                | `8`        | Maximum simultaneous voices       |
| `stealing`  | `VoiceStealingStrategy?` | `"oldest"` | Strategy when all voices are used |

**VoiceStealingStrategy:** `"oldest" | "quietest" | "highest" | "lowest"`

**PolyphonyState:**
| Field       | Type          | Description                                       |
| ----------- | ------------- | ------------------------------------------------- |
| `voices`    | `Voice[]`     | Currently allocated voices (active and releasing) |
| `heldNotes` | `Set<number>` | Currently held note numbers                       |
...
| `id`       | `number`  | Unique voice ID for keying                   |
| `active`   | `boolean` | Whether the note is currently held (gate on) |

#### `midiNoteToFrequency(note: number, tuning?: number): number`
Helper function that converts a MIDI note number to frequency in Hz. Uses A4 = `tuning` (default 440 Hz) as the reference.

---

### Presets

#### `usePresets(options: PresetManagerOptions): PresetManager`
Manages a persistent list of named plugin presets with `localStorage` backing. Supports save, load, delete, rename, export, and import.

**PresetManagerOptions:**
| Field            | Type                                     | Description                                        |
| ---------------- | ---------------------------------------- | -------------------------------------------------- |
| `pluginId`       | `string`                                 | Unique key for namespacing presets in localStorage |
| `getState`       | `() => Record<string, number>`           | Capture current parameter state                    |
| `setState`       | `(data: Record<string, number>) => void` | Restore a parameter state                          |
| `factoryPresets` | `Preset[]?`                              | Default presets loaded when localStorage is empty  |

**PresetManager:**
| Field           | Type                                    | Description                                 |
| --------------- | --------------------------------------- | ------------------------------------------- |
| `presets`       | `Preset[]`                              | Full list of presets (factory + user)       |
| `selectedIndex` | `number`                                | Currently selected preset index (-1 = none) |
| `select`        | `(index: number) => void`               | Select and restore a preset                 |
| `save`          | `(name: string) => void`                | Save current state as a new preset          |
| `delete`        | `(index: number) => void`               | Delete a preset by index                    |
| `rename`        | `(index: number, name: string) => void` | Rename a preset                             |
| `exportPreset`  | `(index: number) => string`             | Serialize a preset to JSON                  |
| `importPreset`  | `(json: string) => void`                | Import a preset from JSON                   |
